...
Last updated: May 2026 — Chat 6 commercial roadmap added
...
bash# 1. Open Codespace: github.com/codespaces → literate space acorn
...
Commit and push all changes (git add . && git commit -m "..." && git push)
Verify Vercel deployment completed successfully
Run the smoke test checklist (below)
Update the handover doc with what was done and what's next
Update this blueprint if any new lessons were learned
...
Chat 6 — Settings Fix + Commercial Planning
Built: Owner-only settings visibility (in progress), interactive day selector (in progress)
Planned: Full commercial readiness roadmap, pricing model, pitch framework
Learned: The right time to think about commercial readiness is when real users are actively using it — which is now. Having Robbie, Nick and Carly as real users is the best possible testing environment.
Do differently: Nothing — this was the right conversation at the right time.
...
The Four Layers
LayerWhat it meansStatusStabilityIt doesn't break🟡 In progressReliabilityIt's always available🔴 Not startedUsabilityA stranger can figure it out🔴 Not startedTrustUsers feel safe putting their work in it🔴 Not started
...
PlanPriceWhat's includedFree$01 show, 1 host, basic planner — gets people in the doorPro$19/month per showUnlimited hosts, radio planner, PDF export, guest book, templatesNetworkCustomMultiple shows, admin dashboard, white-label, priority support
...
Production working before features. Login → dashboard → one core feature, all tested on a real device, before writing anything else.
RLS on from day one. Not as an afterthought. Every table, from the start.
Migrations applied same day. Write it, apply it, commit it. Never let prod drift.
Test on a real device, real network, every session. Not just localhost.
One job per Claude Code prompt. No mixing. No multitasking.
Update the handover doc before closing the session. Always.
